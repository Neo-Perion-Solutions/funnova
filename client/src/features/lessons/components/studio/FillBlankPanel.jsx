...
import FillBlankQuestionModal from './FillBlankQuestionModal';

const FillBlankPanel = ({ lessonId }) => {
  const { data: questions = [], isLoading } = useLessonQuestions(lessonId, 'fill_blank');
  ...
  const handleSave = async (data) => {
    if (selectedQuestion) {
      await update.mutateAsync({
        ...
        data: { ...data, type: 'fill_blank' },
      });
    } else {
      await create.mutateAsync({ ...data, type: 'fill_blank', lesson_id: lessonId });
    }
    ...
    <div className="space-y-4">
      {questions.length >= 20 && (
        <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">
          You've reached the maximum of 20 Fill-in-the-Blank questions
        </div>
      )}
